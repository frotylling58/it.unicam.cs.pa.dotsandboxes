...
public class DumbStrategy implements Strategy {
	...
	private Dot[] getAdjacence(int x, int y, int size) {
		// controllo angoli
		if (x == 0 && y == 0)
			return new Dot[] { new Dot(1, 0), new Dot(0, 1) };
		if (x == 0 && y == size)
			return new Dot[] { new Dot(0, size - 1), new Dot(1, size) };
		if (x == size && y == 0)
			return new Dot[] { new Dot(size - 1, 0), new Dot(size, 1) };
		if (x == size && y == size)
			return new Dot[] { new Dot(size - 1, size), new Dot(size, size - 1) };
		// controllo lati
		if (y == 0)
			return new Dot[] { new Dot(x + 1, y), new Dot(x - 1, y), new Dot(x, y + 1) };
		if (y == size)
			return new Dot[] { new Dot(x + 1, y), new Dot(x - 1, y), new Dot(x, y - 1) };
		if (x == 0)
			return new Dot[] { new Dot(x + 1, y), new Dot(x, y + 1), new Dot(x, y - 1) };
		if (x == size)
			return new Dot[] { new Dot(x - 1, y), new Dot(x, y + 1), new Dot(x, y - 1) };
